...
<table border="1">
	<tr>
		...
		<td class="td_right">
			<c:choose>
			    <c:when test="${not empty article.profile}">
			        <img src="${contextPath}/profile?filename=${article.profile}" width="30px" style="border-radius:50%"/>
			    </c:when>
			    <c:otherwise>
			        <img src="${contextPath}/img/person.jpg" width="30px" style="border-radius:50%"/>
			    </c:otherwise>
			</c:choose>
		<span>${article.writerName }</span></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<tr>
			...
			<td class="td_right">
				<c:if test="${article.imageFilename ne Empty and article.imageFilename ne '' }">
					<img src="${contextPath}/image?filename=${article.imageFilename}" width="100px"/>
				</c:if>
		...
		<tr>
			...
			<td class="td_right"><a href="${contextPath}/fileDown?filename=${article.downloadFilename }">${article.downloadFilename }</a></td>
		</tr>
</table>
<br>
<div id="commandCell">
